...
let
  dataDir = "/mnt/appdata/minecraft/carpetServer";
  ...
  serverJar = "VasCM_v0.1.5.12.jar";
  javaPackage = pkgs.temurin-jre-bin-8;
  openFirewall = true;
  port = 25565;
in
{
  ...
  systemd.services.minecraft-server = {
    ...
    serviceConfig = {
      ExecStart = "${javaPackage}/bin/java ${jvmOpts} -jar ${serverJar} nogui";
      ExecStop =
        let
    ...
    preStart = ''
      ln -sf ${builtins.toFile "eula.txt" "# eula.txt managed by NixOS Configuration\neula=true"} eula.txt
    '';
  };
