...
in
{
  ...
  options = {
    services.homepage-dashboard = {
      ...
      services = lib.mkOption {
        # type = lib.types.attrsOf (lib.types.attrsOf settingsFormat);
        inherit (settingsFormat) type;
        description = ''
  ...
  config =
    let
      services = to2LayerYMLList cfg.services;
    in
    lib.mkIf cfg.enable {
      environment.etc = {
        ...
        "homepage-dashboard/services.yaml".source = settingsFormat.generate "services.yaml" services;
