...
export default class ConfigManager {
  ...
  constructor(configID) {
    if (this.numConfigs === false) {
      this.initialise();
    } else if (this.numConfigs !== 0) {
   ...
   */
  initialise() {
    localStorage.configs = structuredClone(exampleConfigs);
    console.log(exampleConfigs, localStorage.config, localStorage);
  }

  /**
   * @returns {Number | false} The number of saved configs, or false if not initialised
   */
  get numConfigs() {
    if (!Object.hasOwn(localStorage, 'configs')) return false;
    return localStorage.configs.length;
  }
