<script lang="ts">
  ...
  function formatDuration(duration: number): string {
    ...
    // Option 2
    // return `${roundedHours}:${roundedMinutes < 10 ? "0" : ""}${roundedMinutes}:${roundedSeconds < 10 ? "0" : ""}${roundedSeconds}`;
    ...
    // }
    // if (minutes > 0) {
    //   output += `${roundedMinutes}:`;
    // }
    // output += `${roundedSeconds < 10 ? "0" : ""}${roundedSeconds}`;

    // Option 4
    const DECIMAL_PLACES = 2;

    if (hours > 0) {
      output += `${roundedHours.toFixed(DECIMAL_PLACES)} Hours`;
    }
