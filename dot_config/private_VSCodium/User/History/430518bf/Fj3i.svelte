<script lang="ts">
  ...
  function formatDuration(duration: number): string {
    ...
    // Option 2
    // return `${Math.floor(hours)}:${Math.floor(minutes) < 10 ? "0" : ""}${Math.floor(minutes)}:${Math.floor(seconds) < 10 ? "0" : ""}${Math.floor(seconds)}`;

    // Option 3 1234:56:78 or 12:34 or 12
    // if (hours > 0) {
    ...
    // }
    // output += `${Math.floor(seconds) < 10 ? "0" : ""}${Math.floor(seconds)}`;
    // return output;
